...
const STATUS_STYLES = {
  'In Progress': { bg: 'rgba(200, 169, 107, 0.1)', border: 'rgba(200, 169, 107, 0.2)', text: '#C8A96B' },
      ...
      >
        ...
        >
          <div
            ...
            style={{
              background: 'linear-gradient(135deg, rgba(200,169,107,0.05), rgba(13,32,53,0.08))',
  ...
  return (
    <DetailPopup title="Attendance Summary" icon="📅" onClose={onClose}>
      <div className="grid grid-cols-2 gap-3 mb-4">
        {[
          { label: 'Days Present', value: s.thisMonth.daysPresent, color: '#C8A96B' },
          { label: 'Days Absent', value: s.thisMonth.totalDays - s.thisMonth.daysPresent, color: '#f87171' },
          { label: 'Working Days', value: s.thisMonth.totalDays, color: '#DFC993' },
      ...
      <InfoRow label="Avg Hours / Day" value={`${s.thisMonth.avgHours.toFixed(1)}h`} color="#C8A96B" />
  ...
  return (
    <DetailPopup title="Leave Balance" icon="🌴" onClose={onClose}>
      <div className="grid grid-cols-3 gap-3 mb-5">
        {[
          { label: 'Casual', value: lb.casual, color: '#C8A96B' },
      ...
      <InfoRow label="Total Available" value={lb.casual + lb.sick + lb.earned} color="#C8A96B" />
      <InfoRow label="Policy Year" value="2026" />
      <InfoRow label="Carry Forward" value="5 days (Earned)" color="#DFC993" />
    </DetailPopup>
  );
}
  ...
  return (
    <DetailPopup title="Overtime Details" icon="⏰" onClose={onClose}>
      <div className="flex gap-3 mb-5">
        ...
        <div className="flex-1 p-5 rounded-xl text-center" style={{ background: 'rgba(13,32,53,0.06)', border: '1px solid rgba(13,32,53,0.2)' }}>
          <p className="text-2xl font-black" style={{ color: '#0D2035' }}>₹{(ot * 250).toLocaleString()}</p>
...
function SalaryBreakdownPopup({ payslip, onClose }) {
  ...
  return (
    <DetailPopup title={`Salary Breakdown — ${month}`} icon="💰" onClose={onClose}>
      <div className="flex gap-3 mb-5">
        <div className="flex-1 p-4 rounded-xl" style={{ background: 'rgba(200,169,107,0.06)', border: '1px solid rgba(200,169,107,0.15)' }}>
          <p className="text-[10px] text-text-muted">Gross</p>
          <p className="text-xl font-bold" style={{ color: '#C8A96B' }}>{gross}</p>
        </div>
      ...
      <div className="rounded-xl mb-4" style={{ background: 'var(--color-surface-card)', border: '1px solid var(--color-surface-border)' }}>
        <div className="px-4">
          <InfoRow label="Basic Salary" value={`₹${basic.toLocaleString()}`} color="#C8A96B" />
          <InfoRow label="HRA" value={`₹${hra.toLocaleString()}`} color="#0D2035" />
          <InfoRow label="Conveyance" value={`₹${conveyance.toLocaleString()}`} color="#DFC993" />
          <InfoRow label="Special Allowance" value={`₹${special.toLocaleString()}`} color="#123452" />
    ...
    <DetailPopup title={project.name} icon="🚀" onClose={onClose}>
      ...
      <div className="mb-5">
        ...
        <div className="h-3 rounded-full" style={{ background: 'rgba(38,38,47,0.9)' }}>
          <div className="h-full rounded-full" style={{ width: `${project.progress}%`, background: project.progress === 100 ? '#4ade80' : 'linear-gradient(90deg, #C8A96B, #0D2035)', boxShadow: '0 0 10px rgba(200,169,107,0.25)' }} />
        </div>
      </div>
      <div className="rounded-xl" style={{ background: 'var(--color-surface-card)', border: '1px solid var(--color-surface-border)' }}>
        <div className="px-4">
  ...
  return (
    <DetailPopup title={goal.title} icon="🎯" onClose={onClose}>
      ...
      <div className="mb-5">
        ...
        <div className="h-3 rounded-full" style={{ background: 'rgba(38,38,47,0.9)' }}>
          <div className="h-full rounded-full" style={{ width: `${goal.progress}%`, background: goal.progress === 100 ? '#4ade80' : 'linear-gradient(90deg, #C8A96B, #0D2035)' }} />
        </div>
      </div>
      <div className="rounded-xl" style={{ background: 'var(--color-surface-card)', border: '1px solid var(--color-surface-border)' }}>
        <div className="px-4">
...
function GoalsQuickPopup({ onClose }) {
  ...
  return (
    <DetailPopup title="My Q3 Goals" icon="🎯" onClose={onClose}>
      <div className="flex gap-3 mb-5">
        {[
          { label: 'Total', value: goals.length, color: '#C8A96B' },
          { label: 'Done', value: completed, color: '#4ade80' },
          { label: 'In Progress', value: goals.filter(g => g.status === 'In Progress').length, color: '#DFC993' },
        ].map((s, i) => (
...
function ComplaintDetailPopup({ complaint, onClose }) {
  ...
  return (
    <DetailPopup title="Complaint Detail" icon="⚠️" onClose={onClose}>
      ...
      <div className="rounded-xl" style={{ background: 'var(--color-surface-card)', border: '1px solid var(--color-surface-border)' }}>
        <div className="px-4">
          <InfoRow label="ID" value={complaint.id} color="#C8A96B" />
        ...
        <div className="space-y-4">
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {['Casual', 'Sick', 'Earned'].map((t) => (
                <button key={t} onClick={() => setType(t)} className="py-2 rounded-xl text-xs font-semibold transition-all" style={{ background: type === t ? 'rgba(200,169,107,0.15)' : 'rgba(255,255,255,0.03)', border: `1px solid ${type === t ? 'rgba(200,169,107,0.4)' : 'rgba(38,38,47,0.9)'}`, color: type === t ? '#C8A96B' : 'var(--color-text-secondary)' }}>{t}</button>
              ))}
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="date" value={fromDate} onChange={(e) => setFromDate(e.target.value)} className="w-full rounded-xl px-3 py-2 text-xs text-text-primary" style={{ background: 'var(--color-surface-card)', border: '1px solid var(--color-surface-border)' }} />
            </div>
            <div>
              <label className="text-[10px] uppercase tracking-widest text-text-muted block mb-2">To</label>
              <input type="date" value={toDate} onChange={(e) => setToDate(e.target.value)} className="w-full rounded-xl px-3 py-2 text-xs text-text-primary" style={{ background: 'var(--color-surface-card)', border: '1px solid var(--color-surface-border)' }} />
            </div>
          </div>
          <div>
            <label className="text-[10px] uppercase tracking-widest text-text-muted block mb-2">Reason</label>
            <textarea rows={3} value={reason} onChange={(e) => setReason(e.target.value)} className="w-full rounded-xl px-3 py-2 text-xs text-text-primary resize-none" style={{ background: 'var(--color-surface-card)', border: '1px solid var(--color-surface-border)' }} placeholder="Briefly describe your reason..." />
          </div>
          <button onClick={handleSubmit} disabled={!fromDate} className="w-full py-2.5 rounded-xl text-sm font-bold transition-all" style={{ background: fromDate ? 'linear-gradient(135deg, #C8A96B, #0D2035)' : 'rgba(38,38,47,0.9)', color: fromDate ? '#000' : 'rgba(255,255,255,0.3)', cursor: fromDate ? 'pointer' : 'not-allowed' }}>Submit Request</button>
        </div>
      )}
    </DetailPopup>
  );
...
function WeeklyHoursTooltip({ active, payload }) {
  ...
  return (
    <div style={{ background: 'rgba(8,8,12,0.92)', backdropFilter: 'blur(12px)', border: '1px solid var(--color-surface-border)', borderRadius: 12, padding: '8px 14px' }}>
      <p style={{ color: '#C8A96B', fontWeight: 600, fontSize: 13, margin: 0 }}>{d.day}</p>
      <p style={{ color: 'var(--color-text-muted)', fontSize: 12, margin: '4px 0 0' }}>{d.hours}h worked</p>
    </div>
  );
}
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="glass-card p-7 relative overflow-hidden animate-fade-in-up" style={{ opacity: 0 }}>
        <div className="absolute inset-0 opacity-30" style={{ background: 'linear-gradient(135deg, rgba(200, 169, 107, 0.08), rgba(13, 32, 53, 0.08))' }} />
          ...
          <div className="glass-card p-6 animate-fade-in-up delay-300 h-full" id="weekly-hours-chart">
            ...
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={stats.weeklyHours} barSize={28}>
                ...
                <YAxis domain={[0, 12]} tick={{ fill: 'var(--color-text-muted)', fontSize: 10 }} axisLine={false} tickLine={false} width={24} />
            ...
            <div className="flex items-center gap-2 mt-3 text-[10px] text-text-muted">
              <div className="w-3 h-[2px] rounded-full" style={{ background: '#C8A96B' }} /><span>Standard (7–8h)</span>
              <div className="w-3 h-[2px] rounded-full ml-2" style={{ background: '#0D2035' }} /><span>Overtime (9h+)</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="glass-card p-6" id="projects-page">
        ...
        <div className="space-y-4">
          {stats.recentProjects.map((project, i) => {
            const st = STATUS_STYLES[project.status] || STATUS_STYLES['Pending'];
            return (
              <div key={i} className="p-5 rounded-2xl hover:bg-white/[0.04] transition-colors cursor-pointer group" style={{ background: 'var(--color-surface-card)', border: '1px solid var(--color-surface-border)' }} onClick={() => onPopup('project', project)}>
                ...
                <div className="flex items-center gap-3">
                  <div className="flex-1 h-2 rounded-full bg-white/[0.06] overflow-hidden">
                    <div className="h-full rounded-full transition-all duration-700" style={{ width: `${project.progress}%`, background: project.progress === 100 ? '#4ade80' : 'linear-gradient(90deg, #C8A96B, #0D2035)' }} />
                  </div>
    ...
    <div className="space-y-5 animate-fade-in-up" style={{ opacity: 0 }}>
      ...
      <div className="glass-card p-6" id="leave-history">
        <div className="flex items-center justify-between mb-5">
          ...
          <button id="apply-leave-btn" onClick={() => onPopup('apply-leave')} className="px-4 py-2 rounded-xl text-xs font-semibold cursor-pointer transition-all hover:scale-105" style={{ background: 'linear-gradient(135deg, rgba(200,169,107,0.1), rgba(13,32,53,0.1))', border: '1px solid rgba(200,169,107,0.2)', color: '#C8A96B' }}>
            + Apply Leave
          </button>
        </div>
        <div className="space-y-3">
          {leaveHistory.map((item, i) => {
            ...
            return (
              <div key={i} className="flex items-center justify-between p-4 rounded-xl hover:bg-white/[0.03] transition-colors" style={{ background: 'var(--color-surface-card)' }}>
                <div className="flex items-center gap-3">
            ...
            >
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  style={{ background: 'rgba(200,169,107,0.08)', border: '1px solid rgba(200,169,107,0.2)', color: '#C8A96B' }}
                  onClick={(e) => { e.stopPropagation(); }}
                >
                  ↓ Download
                </button>
      ...
      <div className="glass-card p-6" id="goals-page">
        ...
        <div className="space-y-4">
          {goals.map((goal, i) => {
            const st = STATUS_STYLES[goal.status] || STATUS_STYLES['Pending'];
            return (
              <div key={i} className="p-5 rounded-2xl hover:bg-white/[0.04] transition-colors cursor-pointer group" style={{ background: 'var(--color-surface-card)', border: '1px solid var(--color-surface-border)' }} onClick={() => onPopup('goal', goal)}>
                <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center gap-3">
                  <div className="flex-1 h-2 rounded-full bg-white/[0.06] overflow-hidden">
                    <div className="h-full rounded-full transition-all duration-700" style={{ width: `${goal.progress}%`, background: goal.progress === 100 ? '#4ade80' : 'linear-gradient(90deg, #C8A96B, #0D2035)' }} />
                  </div>
